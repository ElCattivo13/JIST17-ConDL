...
for i in {5,10,15,20,25} ; do
	...
	for j in {1..100} ; do
		if [ ! -f "$i/test$i-$j.crom" ] ; then
			...
			/usr/bin/time --quiet -o "$i/benchmark.csv" -a -f "%e;%U;%P;%S;%x" $JAVA -jar org.rosi.crom.benchmark.jar -s$j -n$i -b$b -d$d -rg$rg "$i/test$i-$j.crom" ;
		fi		
	done
	...
	for f in `ls $i/*.crom` ; do
		if [ ! -f "${f/.crom/.owl}" ] ; then	
			echo "${f/.crom/.owl}" ;
			/usr/bin/time --quiet -o "$i/transform.csv" -a -f "%e;%U;%P;%S;%x" $JAVA -jar "org.rosi.crom.toformal.cli.jar" -towl $f
		fi
	done
